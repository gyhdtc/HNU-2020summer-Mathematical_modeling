...
def conv2(x):
    ...
    base_money = np.zeros([30])
    # 每年的本金
    base_money[0] = 8000-students_money
    base_money[1] = store_money[0]-store_money[1]-students_money
...
#     students_money=students_money-i*10
    ...
    b = (0,8000)
    bd = [b for i in range(480)]
    res = minimize(func, np.zeros([480])+10+1e-5, options={'maxiter':100000,'disp':True},constraints=con,bounds=bd)
    x = res['x']
    print(np.round(np.reshape(x,[16,30]),2))
    # print(store_money)
    ...
    # 每年的本金
    base_money[0] = 8000 - students_money
    base_money[1] = base_money[0] - students_money - np.sum(x[..., 0])
    for i in range(1, base_money.shape[0] - 1):
        base_money[i+1] = base_money[i]-students_money-np.sum(x[...,i])+np.sum(adprofit[...,i])
        # base_money[i + 1] = np.sum(adprofit[..., i])
        # print(base_money[i])
    # 每年的投资
    year_invest = np.sum(x, axis=0)  # 30
